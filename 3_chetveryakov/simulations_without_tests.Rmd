...
> Test 1: Simulate and compare the power curves and estimate minimal _N_ for $\sigma = 15$ and $\sigma = 3$.
...
sim_dt <- data.table(expand.grid(n_subjects = 2:200, n_reps = 1:max_n_reps, sigma = sigma_values)) 
...
# lets try to run t-test with N = 20 per group 
n_subjects <- 20

# generate the data
data <- conditions[,.( dv = rnorm(n_subjects, mu, sigma)), by = .(label)]
# compute t-test
ttest_res <- t.test(data[label=='A', dv], data[label=='B', dv])
ttest_summary <- summary(ttest_res)
...
# now, we'll wrap this code in a function for the power calculations
get_t_test_p <- function(n_subjects, conditions){
  data <- conditions[,.( dv = rnorm(n_subjects, mu, sigma)), by = .(label)]
  # compute t-test
  ttest_res <- t.test(data[label=='A', dv], data[label=='B', dv])
  ttest_res$p.value
...
> Test 2: Simulate and compare the power curves and estimate minimal N for a two-sample t-test with $\mu = 5$, $\sigma = 10$ and $\mu = 15$, $\sigma = 10$.
...
> Test 3: Read the data from Raifei et al. (2020) study (`Raifei_2020_simplified.csv`) and estimate the power for 10, 20, 40, and 100 subjects for the average trial N from 100 to 500 in steps of 100 (in both conditions). 
